...
implements OnEditorActionListener, OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        TipDatabaseHandler handler = new TipDatabaseHandler(this);

        Log.d("Insert: ", "Inserting...");
        handler.addTip(new Tip(0, 0, 40.60f, .15f));
        handler.addTip(new Tip(1, 1, 23.50f, .17f));

        Log.d("Reading: ", "Reading all shops...");

        List<Tip> tips = handler.getTips();

        for(Tip tip: tips) {
            String log = "Id: " + tip.getId() + " , Date: " + tip.getDateStringFormatted() +
